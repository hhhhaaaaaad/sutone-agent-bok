...
public interface IArticleLikeDao {
    ...
    @Insert("INSERT IGNORE INTO article_like(article_id, user_id) VALUES(#{articleId}, #{userId})")
    ...
    @Select("SELECT COUNT(1) > 0 FROM article_like WHERE article_id = #{articleId} AND user_id = #{userId}")
